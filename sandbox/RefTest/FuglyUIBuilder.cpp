...
FuglyUIBuilder::FuglyUIBuilder(boost::shared_ptr<lost::common::Config> inConfig)
: UserInterfaceBuilder(inConfig)
{
}


shared_ptr<ControlFactory> FuglyUIBuilder::createControlFactory(boost::shared_ptr<Config> inConfig)
{
  shared_ptr<FuglyFactory> fuglyFactory(new lost::guiro::factory::FuglyFactory(inConfig));
  return fuglyFactory;
}

void FuglyUIBuilder::addControls(boost::shared_ptr<UserInterface> ui, boost::shared_ptr<ControlFactory> factory)
{
  boost::shared_ptr<Label> flabel = factory->createLabel("flabel");
  flabel->setArea(0, 0, 200, 50);
  flabel->caption("testing");
  boost::shared_ptr<Label> flabel2 = factory->createLabel("flabel2");
  flabel2->font().id="9x15";
  flabel2->setArea(0, 20, 200, 50);
  flabel2->caption("testing");

  boost::shared_ptr<ShapeView> shapeView(new ShapeView("shapeView"));
  ...
  boost::shared_ptr<Window> window = factory->createWindow("testWindow", "framedWindow");
