...
static eastl::map<lua_State*, lost::lua::State*> stateMap;
...
namespace lost
{
  namespace lua
  {
      ...
      {
        LoadException(const eastl::string& v) : runtime_error(v.c_str()){};
      };
    ...
    State::~State()
    {
      ...
      eastl::map<lua_State*, lost::lua::State*>::iterator pos;
...
//      DOUT("stateMap size: "<<stateMap.size());
    ...
    eastl::string State::getScriptFilename(const eastl::string& scriptContent, const eastl::string& defaultName)
    {
      eastl::string file(MD5(scriptContent.c_str()).hexdigest().c_str());
      if (fileHashes.find(file) != fileHashes.end()) return fileHashes[file];
        else return defaultName;
    }
    
    eastl::string State::getScriptSource(lua_Debug& debug)
    {
    ...
    eastl::string State::pathFromNamespace(const eastl::string& inNamespace)
    {
      // convert naming convention "lost.lua..." to path "lost/lua/..."
      eastl::string filename(inNamespace);
      size_t pos;
    ...
    int State::doResourceFile(const eastl::string& inRelativePath)
    {
      return doFile(pathFromNamespace(inRelativePath));
    }
    
    void State::addScriptPathEntry(const eastl::string& inScript, const eastl::string& inAbsolutePath)
    {
      fileHashes[MD5(inScript.c_str()).hexdigest().c_str()] = inAbsolutePath;      
    }
    
    // loads and executes a file
    int State::doFile(const eastl::string& inAbsolutePath)
    {
      std::ostringstream os;
      shared_ptr<common::Data> rawData = loader->load(inAbsolutePath.c_str());
    ...
    int State::doString(const eastl::string& inData)
    {
    ...
    eastl::string State::getFilenameFuncnameLine()
    {
      lua_Debug debug;
      string result = "(Lua)";
