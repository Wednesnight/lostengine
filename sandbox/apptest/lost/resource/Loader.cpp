...
namespace lost
{
  namespace resource
  {
    ...
    common::DataPtr Loader::load( const boost::filesystem::path& inPath)
    {
      ...
      if(!result)
        throw LoaderError(eastl::string("couldn't load file: '")+inPath.string().c_str()+"'");

//      DOUT("'"<<inPath << "' [" << result->size << " Bytes]");
      return result;
    }

    eastl::string Loader::locate(const boost::filesystem::path& inPath)
    {
      eastl::string result = inPath.string().c_str();
      for (unsigned int idx = 0; idx < repositories.size(); ++idx)
      {
    ...
    {
      bool result = false;
      eastl::string p = inPath.string().c_str();
      for (unsigned int idx = 0; idx < repositories.size(); ++idx)
      {
