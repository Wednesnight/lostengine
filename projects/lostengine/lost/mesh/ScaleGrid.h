 ...
 * l,r,t,b are absolue pixel coordinates in the bitmap pixel coord space.
...
struct ScaleGrid : public MESHTYPE
{
            ...
            float bottom)
  {
    ...
    this->vertexData[2] = VertexType(right, zr.top());
    this->vertexData[3] = VertexType(zr.topRight());
    this->vertexData[4] = VertexType(zr.left(), top);
    this->vertexData[5] = VertexType(left, top);
    this->vertexData[6] = VertexType(right, top);
    this->vertexData[7] = VertexType(zr.right(), top);
    this->vertexData[8] = VertexType(zr.left(), bottom);
    this->vertexData[9] = VertexType(left, bottom);
    this->vertexData[10] = VertexType(right, bottom);
    ...
    this->vertexData[14] = VertexType(right, zr.bottom());
    this->vertexData[15] = VertexType(zr.bottomRight());

    // now we can use these pixel coordinates as coordinates into the provided texture
    // and use the texture to calculate normalised texture coordinates for the actual mapping
    for(uint32_t i=0; i<numVertices; ++i)
    {
      this->texCoordData[i] = tex->normalisedCoord(this->vertexData[i]);
    }
