...
namespace lost
{
  namespace application
  {
    ...
    void dispatchKeyEvent(Tasklet* tasklet, int code, char* character, bool pressed, bool repeat)
    {
      KeyEventPtr event(new KeyEvent(pressed ? KeyEvent::KEY_DOWN() : KeyEvent::KEY_UP()));
      event->window    = tasklet->window;
    ...
    void dispatchMouseEvent(Tasklet* tasklet, event::Type which, int x, int y, int absX, int absY, int buttonCode, bool pressed)
    {
    ...
    void Tasklet::run()
    {
	  ...
	  XdndDrop = XInternAtom(display, "XdndDrop", False);
	    ...
	    if (window != NULL) {
	      ...
	      if (waitForEvents || XPending(display) > 0) {
		do {
		  ...
		  switch (event.type) {
			...
			eventDispatcher->dispatchEvent(ResizeEvent::create(event.xconfigure.width, event.xconfigure.height));
		      }
		      break;
		    ...
		    case ButtonRelease:
		      currentButton = 0;
		      dispatchMouseEvent(this, event.type == ButtonPress ? MouseEvent::MOUSE_DOWN() : MouseEvent::MOUSE_UP(), event.xbutton.x, event.xbutton.y,
		    ...
		    case KeyRelease:
		      ...
		      dispatchKeyEvent(this, key, text, event.type == KeyPress, false);
		      break;

		    case MotionNotify:
		      dispatchMouseEvent(this, MouseEvent::MOUSE_MOVE(), event.xmotion.x, event.xmotion.y,
					 event.xmotion.x_root, event.xmotion.y_root, currentButton, currentButton != 0);
		      break;

		    case ClientMessage:
		      if (event.xclient.data.l[0] == WM_DELETE_WINDOW) {
			eventDispatcher->dispatchEvent(WindowEvent::create(WindowEvent::CLOSE(), window));
		      }
