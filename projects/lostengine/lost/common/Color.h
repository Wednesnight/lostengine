...
namespace lost
{
  namespace common
  {
    ...
    struct Color
    {
      ...
      inline uint8_t ru8() const { return (uint8_t)(fv[0]*255); };
      inline uint8_t gu8() const { return (uint8_t)(fv[1]*255); };
      inline uint8_t bu8() const { return (uint8_t)(fv[2]*255); };
      inline uint8_t au8() const { return (uint8_t)(fv[3]*255); };

      inline void ru8(uint8_t v) { fv[0] = float(v)/255.0f; }
      inline void gu8(uint8_t v) { fv[1] = float(v)/255.0f; }
      inline void bu8(uint8_t v) { fv[2] = float(v)/255.0f; }
      inline void au8(uint8_t v) { fv[3] = float(v)/255.0f; }
